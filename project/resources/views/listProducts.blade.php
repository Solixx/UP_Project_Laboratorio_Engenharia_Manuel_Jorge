              ...
              @forelse ($products as $product)
                <tr>
                  ...
                  <td class="border-bottom-0">
                    @if($product->getColorsWithTrashed &&
                        $product->getColorsWithTrashed->first() && 
                        $product->getColorsWithTrashed->first()->photos->first() && 
                        $product->getColorsWithTrashed->first()->photos->first()->exists())
                      <img style="width: 100px; 
                            heigth: 100%; 
                            object-fit: contain; 
                            bject-position: 50% 50%;" 
                            src="{{ asset($product->getColorsWithTrashed->first()->photos->first()->imgPath) }}" 
                            class="card-img-top rounded-0" 
                            alt="{{ $product->getColorsWithTrashed->first()->photos->first()->img }}" />
                    @else
                      <img style="width: 100px; 
                            heigth: 100%; 
                            object-fit: contain; 
